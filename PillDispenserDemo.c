/*
Demo v2 cleans up print statements for easier reading during demo. 
	More comments are added. 
Demo v1 modifies v4 to have the same start up and editing process 
	but uses a count verse the actual time to simulate quicker. 
Demo only changes updateNow() function for determining current day, 
	hour and minute.
Below is the version history of the actual simulation.
...
int day = 0, hour = 0, min = 0; // Demo starts on Sunday at 00:00. 
void updateNow(void); // Updates the current time. 
int timeCounter = -15; // used in demo code
//--------------------------------------------------------------------
...
use this time should be 5 minutes. For demonstration purposes, the time 
is decreased to 10 seconds to allow for a quicker simulation. 
*/
#define WAIT 5
...
#define WAIT2 0.5 // Wait time for user input. Shorter for demo.
//--------------------------------------------------------------------
